                                            ...
                                            uint8_t *query)
{
	...
	case 67:
		if (query[6]>5) query[6]=5;
		if (query[3]==0x0) length=53*query[6];
		else length=60*query[6];
		break;
...
static int read_archives(modbus_param_t *mb_param, int slave, int function,
			  int type, int nb, int hours,
			  int days, int month, int years, uint16_t *data_dest)
{
        int ret;
        int query_length;
        uint8_t query[MIN_QUERY_LENGTH];
